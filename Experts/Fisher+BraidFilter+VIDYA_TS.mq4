...
void OnTick()
  {
   ...
   double stop_loss = NormalizeDouble(1.8 * atr, 5);
  ...
  // Trailing stop
   ...
   for (int b=OrdersTotal()-1;b>=0;b--)
   {
      ...
      if(OrderType() == OP_BUY)
      {
         ...
         if(Bid - OrderOpenPrice() >  50*Point && Bid > OrderOpenPrice() && stopLoss_TS > OrderStopLoss())
         {
      ...
      else if(OrderType() == OP_SELL)
      {
         ...
         if(OrderOpenPrice() - Ask >  50*Point && OrderOpenPrice() > Ask && stopLoss_TS < OrderStopLoss())
         {
            // Modify the stop loss level of the open order
